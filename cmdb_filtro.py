...
for device in my_devices:
    ...
    url_get = url + '?sysparm_query=serial_number%3D{}&sysparm_limit=1'
    response = requests.get(url_get.format(serial), auth=(username, password), headers=headers)
    data = response.json()
    if len(data['result']) == 0:
        ...
        print("No se agregó el equipo {} con número de serie {} debido a que ya existe".format(model, serial))
